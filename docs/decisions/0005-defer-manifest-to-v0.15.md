# ADR-0005: Defer `brimley manifest` to v0.15
...
- Implementing manifest before the function type surface is stable (i.e., before Plugin Architecture lands in v0.14) risks needing schema revisions as new runner types are added.
...
Defer `brimley manifest` to a new dedicated release: **v0.15**, after Plugin Architecture (v0.14). This ensures the manifest schema captures the complete, stable set of function types including community-contributed runners.
...
**Positive:**
- The manifest schema is defined once against a complete, stable function type surface. No iterative revisions as new runners are added in v0.7 through v0.14.
- All releases from v0.7 to v0.14 remain focused on runtime features rather than tooling.
- v0.15 becomes a coherent "integration & export" release that pairs well with the community ecosystem landing in v0.14.

**Trade-offs:**
- External Copilot platform integrations that rely on schema export cannot use an official `brimley manifest` command until v0.15. Teams needing this earlier must generate schema manually or via custom tooling. Given that the MCP integration (`mcp:` block on each function) covers the primary LLM-facing exposure use case, this gap affects only teams needing static schema artifacts for non-MCP platforms.
