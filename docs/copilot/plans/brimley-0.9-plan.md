...
| B09-S9 | In Progress | Three-phase shutdown | Modify `BrimleyContainer.shutdown()`: insert `TaskScheduler.stop()` (30s grace → hard cancel) before hooks and teardown; no-op when no tasks | `tests/test_task_scheduler.py` (graceful stop, hard cancel after timeout); `tests/test_container.py` (three-phase ordering, backward compat) |
