...
| B09-S6 | Completed | TaskScheduler core (daemon thread + event loop) | New `core/task_scheduler.py`: `TaskScheduler` class with `start()`, `stop()`, periodic execution loop, `immediate` handling | `tests/test_task_scheduler.py` (start/stop, periodic execution, immediate flag, not started in invoke mode) |
| B09-S7 | Completed | Retry policy implementation | `TaskScheduler` retry logic: exponential/multiplier/fixed backoff, ceiling at `interval`, reset on success, exhaustion handling | `tests/test_task_scheduler.py` (retry formats, backoff ceiling, reset, exhaustion, unlimited retries) |
| B09-S8 | Completed | Overlap prevention and diagnostic warning | `TaskScheduler` overlap guard: skip iteration if previous running; `consecutive_skip_count` tracking; 3-skip WARNING | `tests/test_task_scheduler.py` (overlap skip, diagnostic warning at 3 skips, counter reset, manual invoke bypasses guard) |
| B09-S9 | Completed | Three-phase shutdown | Modify `BrimleyContainer.shutdown()`: insert `TaskScheduler.stop()` (30s grace → hard cancel) before hooks and teardown; no-op when no tasks | `tests/test_task_scheduler.py` (graceful stop, hard cancel after timeout); `tests/test_container.py` (three-phase ordering, backward compat) |
| B09-S10 | Completed | Startup sequence integration | Wire `TaskScheduler` into boot path: instantiate after container ready, query registry for task functions, `start()` after `@on_startup` hooks (only in repl/mcp-serve modes) | `tests/test_startup.py` (task scheduler started in repl/mcp-serve, skipped in invoke, start after hooks) |
| B09-S11 | Completed | `/tasks` REPL admin command | New REPL command showing task name, interval, state (running/waiting/backoff), last run, failure count, next scheduled run | `tests/test_repl_commands.py` or `tests/test_tasks_command.py` (output format, states, empty list) |
| B09-S12 | Completed | Hot-reload warning for scheduling changes | `PartitionedReloadEngine` detects task metadata changes on re-scan; emits warning; does not apply scheduling changes | `tests/test_reload.py` or `tests/test_task_reload.py` (warning emitted, schedule unchanged, logic reloaded) |
| B09-S13 | Completed | `mcp-serve` stdio transport support | Wire `mcp.transport` config value through to `mcp_server.run()`; add `--transport` CLI flag override; REPL always SSE | `tests/test_mcp_provider.py` or `tests/test_mcp_transport.py` (stdio config, CLI flag override, REPL SSE enforcement) |
| B09-S14 | Completed | ADR-0006 and ADR-0007 | Create `docs/decisions/0006-application-server-boundary.md` and `docs/decisions/0007-managed-tasks-design-decisions.md` | Docs review |
| B09-S15 | Completed | Public API exports and example files | Update `__init__.py` exports (remove embedding); new example file demonstrating a task function | `tests/test_packaging_contract.py` (export assertions); example runs via `brimley invoke` |
| B09-S16 | Completed | Documentation updates | Update all docs per Section 6 table: high-level design, python functions, functions, REPL admin commands, configuration, discovery spec, security note, `brimley build` experimental marking | Docs conformance review |
| B09-S17 | Completed | Version bump, CHANGELOG, doc scan gate | `pyproject.toml` → 0.9.0; `CHANGELOG.md`; stale version refs; reference maps; `examples/README.md` | Full suite pass |
| B09-S18 | Completed | Final validation and release gate | Full test suite; regression check; review approval | Full suite pass |
...
Record results:
- Focused: pass — 33 task_scheduler + 13 task_discovery + 35 time_parser + 38 task_models/decorators + 15 tasks_command + 17 task_reload + 10 mcp_transport = 161 focused tests
- Regression: pass — container, startup, execution_python, packaging_contract, mcp_provider, context, discovery all green
- Full suite: **977 passed, 0 failed** (3/29/2026)
...
| B09-S6/S7/S8 | 3/29/2026 | Created `src/brimley/core/task_scheduler.py` with `TaskScheduler` class: daemon thread, own asyncio event loop, `start()`/`stop()`/`get_task_status()`. Clock-and-execute design: `_task_clock()` fires every interval, `_fire_iteration()` launches fire-and-forget `_run_iteration()`. Retry logic in `_run_iteration()` with exponential/multiplier/fixed backoff via `_compute_backoff()`. Overlap guard in `_task_clock()` via `is_executing` flag, skip counter, and WARNING after 3 consecutive skips. Created `tests/test_task_scheduler.py`: 33 tests covering all S6/S7/S8 requirements. | Combined S6/S7/S8 into one implementation pass. Used fire-and-forget pattern for overlap detection (sequential loop design was unsuitable). | 33/33 pass; 145/145 regression tests pass. |
| B09-S9 | 3/29/2026 | Modified `BrimleyContainer.__init__` to add `_lifecycle_hooks: List[Any]` and `task_scheduler: Optional[Any]`. Modified `startup()` to store all lifecycle hooks. Refactored `shutdown()` into three phases: Phase 1 (`task_scheduler.stop()`), Phase 2 (reverse `@on_shutdown` hooks), Phase 3 (existing singleton teardown). Phase 1 and 2 exceptions are logged but do not abort subsequent phases. Added `TestThreePhaseShutdown` class to `test_container.py` with 7 new tests. | None | 46/46 container tests pass; regression clean. |
| B09-S10 | 3/29/2026 | Added `_setup_task_scheduler(scan_result, context, dispatcher, *, start)` helper to `cli/main.py` (locally imports `TaskScheduler`, filters task functions from registry, registers on `context.container.task_scheduler`, calls `start()` iff `start=True`). Imported and called in `repl.py` `load()` (`start=True`) and in `mcp_serve` command (`start=True`). Wired into `invoke` command (`start=False`). Created `tests/test_startup.py` with 11 tests across three classes: start-mode, invoke-mode, and edge cases. | Patch target for `TaskScheduler` in tests is `brimley.core.task_scheduler.TaskScheduler` (not `brimley.cli.main.TaskScheduler`) because the class is imported locally inside the helper function — discovered after initial patch target failed. | 11/11 startup tests pass; 977 full suite pass. |
| B09-S11 | 3/29/2026 | Added `"tasks": self._cmd_tasks` entry to REPL handlers dict. Added `/tasks` to help output. Implemented `_cmd_tasks(self, args)`: reads `context.container.task_scheduler.get_task_status()`, renders a columnar table (NAME, INTERVAL, STATE, FAILURES, LAST RUN, NEXT RUN); handles missing scheduler and empty task list. Created `tests/test_tasks_command.py` with 15 tests: `TestCmdTasksNoScheduler` (3), `TestCmdTasksOutput` (11), `TestTasksCommandRouting` (1). | None. | 15/15 tasks command tests pass; 977 full suite pass. |
| B09-S12 | 3/29/2026 | Modified `PartitionedReloadEngine.apply_reload_with_policy()`: added `pre_swap_functions = list(context.functions)` snapshot before the function registry swap. Renamed detection method to `_detect_task_metadata_changes_from_snapshot(pre_swap_fns, new_fns)` and rewired call. Method compares new task functions against pre-swap snapshot: emits `WARN_NEW_TASK_FUNCTION` for brand-new task functions or functions that gained a `task` field; emits `WARN_TASK_SCHEDULE_CHANGED` when any `TaskConfig` field (interval/immediate/retries/retry_interval) differs. Created `tests/test_task_reload.py` with 17 tests: `TestDetect` (14) and `TestIntegration` (3). | Initial design called `_detect_task_metadata_changes()` post-swap (comparing against the already-updated registry), which made before/after comparison impossible. Fixed by renaming to `_from_snapshot` variant and passing the pre-swap list explicitly. | 17/17 reload tests pass; 977 full suite pass. |
| B09-S13 | 3/29/2026 | Added `--transport` / `--transport=` token parsing to `mcp_serve` in `cli/main.py`. Validated against `{"sse", "stdio"}` with fatal error on unrecognised values. Added `effective_transport = transport_override if transport_override is not None else context.mcp.transport` resolution (CLI override → config → default `"sse"`). Branched the FastMCP serve call: stdio path calls `mcp_server.run(transport="stdio")` with no host/port; SSE path passes host and port as before. Created `tests/test_mcp_transport.py` with 10 tests across `TestTransportArgParsing`, `TestTransportResolution`, and `TestTransportRunCall`. | None. | 10/10 transport tests pass; 977 full suite pass. |
| B09-S14 | 3/29/2026 | Created `docs/decisions/0006-application-server-boundary.md` (event loop contention context, application server decision, positive consequences and trade-offs). Created `docs/decisions/0007-managed-tasks-design-decisions.md` (five design decisions: tasks-as-functions, scheduler-only overlap guard, three-phase shutdown ordering, 30-second global grace, immutable scheduling metadata). Updated `docs/decisions/README.md` index with two new rows. | None. | Docs review; no automated tests. |
| B09-S15 | 3/29/2026 | Created `examples/task_reconciler.py` demonstrating `@function(task={...})` with `@provider` singleton, `@on_startup` seed hook, and `Depends()` injection. Updated `examples/README.md`: bumped baseline header to 0.9.x, added "What's New in 0.9" section, added example entry #10 with REPL and one-shot invocation instructions, added file structure entry. Verified `BrimleyRuntimeController` absent from both `brimley.__all__` and `brimley.runtime.__all__` (done in B09-S2). | `task_reconciler.py` initially used `ledger: dict = Depends("ledger")` and `-> dict` return type — both rejected by the Brimley schema system (`ERR_PARSE_FAILURE`). Fixed by dropping the `dict` annotation on the `Depends` parameter and returning `json.dumps(result)` (`-> str`). Caught in B09-S18. | 5/5 packaging contract tests pass; 24/24 e2e examples tests pass. |
| B09-S16 | 3/29/2026 | Updated 9 docs files: `brimley-high-level-design.md` (section J: TaskScheduler/managed tasks); `brimley-python-functions.md` (section 9: `task=` parameter, task parameters table, retry formats, quarantine rules, scheduling behavior; baseline 0.8.x→0.9.x); `brimley-functions.md` (Task Functions subsection); `brimley-repl-admin-commands.md` (`/tasks` table row, output format section); `brimley-configuration.md` (transport note updated for 0.9, `--transport` CLI override listed, task config per-function clarification); `brimley-discovery-and-loader-specification.md` (quarantine rules table, baseline 0.8.x→0.9.x); `docs/security/brimley-0.7-threat-model.md` (section 5: task trust boundary); `brimley-cli-and-repl-harness.md` (`mcp-serve` signature updated, `--transport` flag documented); `copilot-docs-reference.md` (managed tasks topic row, three new keyword index entries). | `brimley build` experimental marking was already present in the CLI harness doc and the high-level design; no additional change needed. | Docs conformance review; 977 full suite pass. |
| B09-S17 | 3/29/2026 | Bumped `pyproject.toml` version `0.8.1` → `0.9.0`. Added complete `[0.9.0]` section to `CHANGELOG.md` with Added (Managed Tasks, TaskScheduler, human-time parser, quarantine rules, three-phase shutdown, `/tasks`, `--transport`, hot-reload warning, ADR-0006/0007, `task_reconciler.py` example), Changed (three-phase shutdown ordering, hot-reload detection, version bump), and Removed (`BrimleyRuntimeController` from public API) sub-sections. | None. | 977 full suite pass. |
| B09-S18 | 3/29/2026 | Fixed `examples/task_reconciler.py`: removed `dict` type annotation from `ledger=Depends("ledger")` (bare `dict` unsupported by schema system); changed return type `-> dict` to `-> str`; added `import json`; wrapped result in `json.dumps(result)`. Ran full suite: 977 passed, 0 failed. Updated Implementation Plan table: all B09-S10 through B09-S18 step statuses set to `Completed`. | `task_reconciler.py` annotation bug (`dict` annotation on a `Depends` parameter) caused `ERR_PARSE_FAILURE` scanner diagnostic that broke two e2e_examples tests. The fix was to let `Depends` parameters be untyped (the container resolves by name, not type) and use `-> str` with serialization. | 977/977 tests pass, 0 failed. |
